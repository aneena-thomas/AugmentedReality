...
{
    /// Data source
    @objc open weak var dataSource: ARDataSource?
    /// Orientation mask for view controller. Make sure orientations are enabled in project settings also.
    @objc open var interfaceOrientationMask: UIInterfaceOrientationMask = UIInterfaceOrientationMask.all
    /**
     ...
     *       Max value is 10.
     */
    @objc open var maxVerticalLevel = 0
        {
        didSet
        {
    ...
    @objc open var maxVisibleAnnotations = 0
        {
        didSet
        {
    ...
    @objc fileprivate(set) open var trackingManager: ARTrackingManager = ARTrackingManager()
    ...
    @objc open var closeButtonImage: UIImage?
    {
        didSet
        {
    ...
    @objc @available(*, deprecated, message: "Will be removed in next version, use uiOptions.debugEnabled.")
     ...
     */
    @objc open var headingSmoothingFactor: Double = 1
     ...
     */
    @objc open var onDidFailToFindLocation: ((_ timeElapsed: TimeInterval, _ acquiredLocationBefore: Bool) -> Void)?
    ...
    @objc internal func initializeInternal()
    {
        if self.initialized
        {
            return
    ...
    func alertPromptToAllowCameraAccessViaSetting() {
        let alert = UIAlertController(title: AugmentedReality.Error.rawValue, message: AugmentedReality.cameraMessage.rawValue, preferredStyle: UIAlertControllerStyle.alert)
        
        alert.addAction(UIAlertAction(title: AugmentedReality.Cancel.rawValue, style: .default))
        alert.addAction(UIAlertAction(title: AugmentedReality.Settings.rawValue, style: .cancel) { (alert) -> Void in
     ...
     */
    @objc open func setAnnotations(_ annotations: [ARAnnotation])
    {
    ...
    @objc open func getAnnotations() -> [ARAnnotation]
    {
    ...
    @objc open func reloadAnnotations()
    {
    ...
    open class func createCaptureSession() -> (session: AVCaptureSession?, error: NSError?)
    {
        ...
        let videoDevicess = videoDevices
        
            for captureDevice in videoDevicess
            {
    ...
    @objc internal func layoutAndReloadOnOrientationChange()
    {
        CATransaction.begin()
    ...
    @objc func addCloseButton()
    {
        ...
        closeButton.frame = CGRect(x: 0, y: 5,width: 40,height: 40)
    ...
    @objc func CreateToast()
    {
        ...
        let toastLabel = UILabel(frame: CGRect(x:100, y:self.view.frame.origin.y+60, width:self.view.frame.width/2, height:31))
        toastLabel.backgroundColor = UIColor.clear
        toastLabel.textColor = UIColor.red
        toastLabel.textAlignment = NSTextAlignment.center
        self.view.addSubview(toastLabel)
        toastLabel.text = "Move Your Camera"
        toastLabel.font = toastLabel.font.withSize(20)
        toastLabel.adjustsFontSizeToFitWidth = true
        toastLabel.numberOfLines = 0
        toastLabel.alpha = 1.0

        UIView.transition(with: toastLabel, duration: 15.0, options:.transitionCurlUp, animations: { () -> Void in
            toastLabel.alpha = 0.0
//            toastImage.alpha = 0.0
            //            toastimglbl.frame.origin.x = (self.frame.bounds.width / 2) + 12.0
        }, completion:nil)
    }
    //==========================================================================================================================================================
    //MARK:                                                        Debug
